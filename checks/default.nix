...
let
  ...
  launcherResults = [
    ...
    (check "launcher/the-quick-view-is-flat-but-still-knows-every-machine"
      (let t = launcher-basic.nixremote.launcher.rendered.quick; in
      lib.hasInfix "--flat" t
        && lib.hasInfix "archlxc:" t && lib.hasInfix "devhome:" t)
      "the quick view drops the CATEGORIES, and only the categories. Declaring the local mode alone was tried and produced a launcher that had silently forgotten the other machines -- and only on the keystroke, not on the bar button, which is a confusing way for a launcher to behave")

    (check "launcher/the-quick-view-warms-nothing-on-open"
      (!(lib.hasInfix ">/dev/null 2>&1 &" launcher-basic.nixremote.launcher.rendered.quick))
      "it sits on a keystroke: opening must cost a local filesystem read and nothing else, with a peer's inventory fetched when its tab is actually clicked")
