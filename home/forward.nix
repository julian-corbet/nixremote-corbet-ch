  ...
  peerModule = { name, ... }: {
    options = {
      ...
      hostPublicKey = lib.mkOption {
        ...
        description = ''
          This peer's SSH host public key, as one `<type> <base64>` pair (no trailing comment).
          When set, an entry is generated under the peer's `HostKeyAlias` and OpenSSH is pointed at
          it, so the peer verifies on a machine that has never connected to it before.

          THIS EXISTS BECAUSE `HostKeyAlias` BREAKS THE USUAL WORKFLOW, and the failure is a bad
          one. Every generated block pins `HostKeyAlias <sshAlias>` on purpose, so the key stays
          keyed on the stable peer name rather than on whichever address answered — that is what
          lets one peer be reached over LAN today and an overlay tomorrow without a key warning.
          The cost is that an ordinary `known_hosts` entry, which is keyed on the ADDRESS, does not
          satisfy it: connecting to the alias fails with `Host key verification failed` even though
          the very same host, by IP, is already trusted and connects fine.

          Measured, not theorised: on a host where one peer's alias entry existed and another's did
          not, the first peer's application list loaded and the second reported the machine as
          unreachable — with nothing in the failure naming host keys, because the caller only saw
          an empty inventory.

          `null` generates nothing and leaves verification entirely to whatever the user's own
          `known_hosts` already says, which is the right default for a peer that is already
          trusted. Read a key with `ssh-keyscan -t ed25519 <address>`, and verify it out of band
          before declaring it — this option is a trust assertion, and writing it down is exactly as
          strong as the check you did before writing it.
        '';
      };
...
in
{
  ...
  config = lib.mkIf (cfg != { }) {
    ...
    home.file.".ssh/conf.d/nixremote.conf".text =
      let
        # Peers that declared a key get their own known_hosts file; OpenSSH is told to read it IN
        # ADDITION to the user's own, never instead of it -- `UserKnownHostsFile` takes a list, and
        # naming only ours would make every host the user already trusts unknown again.
        keyedPeers = lib.filterAttrs (_: p: p.hostPublicKey != null) cfg;
        knownHostsFile = "${config.home.homeDirectory}/.ssh/conf.d/nixremote_known_hosts";
        knownHostsLine =
          lib.optionalString (keyedPeers != { })
        ...
        mkBlock = peer: n: idx: addr:
          ...
          in ''
            ...
            ${knownHostsLine}${portLine}${userLine}'';
    ...
    # The keys themselves, keyed on each peer's HostKeyAlias -- which is what the generated blocks
    # above verify against. Written only when at least one peer declared one; a file of comments
    # alone would be harmless but would also imply this module manages verification on hosts where
    # it does not.
    home.file.".ssh/conf.d/nixremote_known_hosts" = lib.mkIf
      (lib.any (p: p.hostPublicKey != null) (lib.attrValues cfg))
      {
        text = ''
          # Generated by nixremote (nixremote.forward.<peer>.hostPublicKey) -- do not hand-edit.
          # Keyed on each peer's HostKeyAlias, not on an address: the alias is the stable name the
          # generated ssh blocks pin, precisely so a peer reached over LAN today and an overlay
          # tomorrow verifies against one entry rather than two.
        '' + lib.concatStrings (lib.mapAttrsToList
          (_: peer: "${peer.sshAlias} ${peer.hostPublicKey}\n")
          (lib.filterAttrs (_: p: p.hostPublicKey != null) cfg));
      };
  };
}
